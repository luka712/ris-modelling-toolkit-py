...
def compute_split_points_from_uv_overflow(mesh: trimesh.Trimesh) -> list[UVBoundaryPointInfo]:
    """
    Find points on the mesh where UVs cross integer boundaries outside [0,1] range.
    These points can be used to split the mesh for proper UV mapping.
    ...
    4. Collect all such points for potential mesh splitting.
    ...
    for face in mesh.faces:
        ...
        # find positions
        pos_a = mesh.vertices[face[0]]
        pos_b = mesh.vertices[face[1]]
        pos_c = mesh.vertices[face[2]]

        # find tex coordinates
        tc_a = mesh.visual.uv[face[0]]
        tc_b = mesh.visual.uv[face[1]]
        tc_c = mesh.visual.uv[face[2]]

        # now find points to insert based on uv's and positions.

        # for each uv edge, find uv's boundaries crossed outside of 0-1 range which are integer values
        ...
        a_b_x = find_uv_outside_bounds(tc_a[0], tc_b[0], Axis2D.X)
        a_b_y = find_uv_outside_bounds(tc_a[1], tc_b[1] , Axis2D.Y)

        # Go from b -> c
        b_c_x = find_uv_outside_bounds(tc_b[0], tc_c[0], Axis2D.X)
        b_c_y = find_uv_outside_bounds(tc_b[1], tc_c[1], Axis2D.Y)

        # Go from c -> a
        c_a_x = find_uv_outside_bounds(tc_c[0], tc_a[0], Axis2D.X)
        c_a_y = find_uv_outside_bounds(tc_c[1], tc_a[1], Axis2D.Y)
        ...
        p_ab_x = compute_uv_boundary_points(pos_a, pos_b, tc_a, tc_b, Axis2D.X, a_b_x)
        p_ab_y = compute_uv_boundary_points(pos_a, pos_b, tc_a, tc_b, Axis2D.Y, a_b_y)

        p_bc_x = compute_uv_boundary_points(pos_b, pos_c, tc_b, tc_c, Axis2D.X, b_c_x)
        p_bc_y = compute_uv_boundary_points(pos_b, pos_c, tc_b, tc_c, Axis2D.Y, b_c_y)

        p_ca_x = compute_uv_boundary_points(pos_c, pos_a, tc_c, tc_a, Axis2D.X, c_a_x)
        p_ca_y = compute_uv_boundary_points(pos_c, pos_a, tc_c, tc_a, Axis2D.Y, c_a_y)
        ...
        for uv_boundary_point_info_list in [p_ab_x, p_ab_y, p_bc_x, p_bc_y, p_ca_x, p_ca_y]:
            if uv_boundary_point_info_list is not None:
                ...
                for uv_boundary_point_info in uv_boundary_point_info_list:
                    points.append(uv_boundary_point_info.point)

    return points


def split_mesh_at_points(mesh: trimesh.Trimesh, points: np.ndarray[tuple[float]]) -> trimesh.Trimesh:
    """
    Split a mesh at given points.
    :param mesh: trimesh.Trimesh to be split.
    :param points: The 3D points where splits should occur.
    ...
    for point in points:
        ...
        for idx, face in enumerate(mesh.faces):
            ...
            if point_in_triangle(point, a, b, c):
                ...
                mesh = split_triangle(mesh, idx, point)
                break  # move to next point

    return mesh

def split_triangle(mesh: trimesh.Trimesh, face_index: int, point: np.ndarray[tuple[float]]) -> trimesh.Trimesh:
    """
    Replace one triangle with three new triangles by inserting a vertex.
    Parameters:
    - mesh: trimesh.Trimesh
    - face_index: index of the face to split
    - point: 3D point to insert as new vertex
    Returns:
